...
class TankLayoutRenderer {
  ...
  drawTank(x, y, tank, scale, isHovered = false) {
    ...
    const height = (tank.height || 2000) * scale; // Default height for 3D
    
    if (this.options.is3D) {
      this.drawTank3D(x, y, tank, width, length, height, theme, isHovered);
    } else {
      this.drawTank2D(x, y, tank, width, length, theme, isHovered);
    }
  }
  
  /**
   * Draw tank in 2D (original method)
   */
  drawTank2D(x, y, tank, width, length, theme, isHovered) {
    ...
    this.ctx.lineWidth = Math.max(1, this.calculateScale(this.currentTanks) * 0.5);
    this.ctx.strokeRect(x, y - length/2, width, length);
    
    // Draw tank labels
    this.drawTankLabels(x + width/2, y, tank, theme, width, length);
    
    // Draw tank details if large enough
    if (width > 40 && length > 50 && this.options.showEditButtons) {
      this.drawTankDetails(x, y, tank, width, length, this.calculateScale(this.currentTanks), isHovered, theme, Math.min(12, Math.max(8, this.calculateScale(this.currentTanks) * 8)));
    }
  }
  
  /**
   * Draw tank in 3D isometric view
   */
  drawTank3D(x, y, tank, width, length, height, theme, isHovered) {
    const isoX = 0.5;
    const isoY = 0.25;
    
    // Calculate isometric points
    const frontTopLeft = { x: x, y: y - length/2 };
    const frontTopRight = { x: x + width, y: y - length/2 };
    const frontBottomLeft = { x: x, y: y + length/2 };
    const frontBottomRight = { x: x + width, y: y + length/2 };
    
    const backTopLeft = { x: x + height * isoX, y: y - length/2 - height * isoY };
    const backTopRight = { x: x + width + height * isoX, y: y - length/2 - height * isoY };
    const backBottomLeft = { x: x + height * isoX, y: y + length/2 - height * isoY };
    const backBottomRight = { x: x + width + height * isoX, y: y + length/2 - height * isoY };
    
    // Draw back faces (darker)
    this.ctx.fillStyle = this.darkenColor(theme.tankColor, 0.4);
    
    // Top face
    this.ctx.beginPath();
    this.ctx.moveTo(frontTopLeft.x, frontTopLeft.y);
    this.ctx.lineTo(frontTopRight.x, frontTopRight.y);
    this.ctx.lineTo(backTopRight.x, backTopRight.y);
    this.ctx.lineTo(backTopLeft.x, backTopLeft.y);
    this.ctx.closePath();
    this.ctx.fill();
    this.ctx.strokeStyle = theme.tankBorder;
    this.ctx.stroke();
    ...
    // Right face
    this.ctx.fillStyle = this.darkenColor(theme.tankColor, 0.2);
    this.ctx.beginPath();
    this.ctx.moveTo(frontTopRight.x, frontTopRight.y);
    this.ctx.lineTo(frontBottomRight.x, frontBottomRight.y);
    this.ctx.lineTo(backBottomRight.x, backBottomRight.y);
    this.ctx.lineTo(backTopRight.x, backTopRight.y);
    this.ctx.closePath();
    this.ctx.fill();
    this.ctx.stroke();
    ...
    // Draw tank labels on front face
    this.drawTankLabels(x + width/2, y, tank, theme, width, length);
    
    // Draw tank details if large enough
    if (width > 40 && length > 50 && this.options.showEditButtons) {
      this.drawTankDetails3D(x, y, tank, width, length, height, this.calculateScale(this.currentTanks), isHovered, theme);
    }
  }
  
  /**
   * Draw tank labels (shared between 2D and 3D)
   */
  drawTankLabels(centerX, centerY, tank, theme, width, length) {
    ...
    this.ctx.textAlign = 'center';
    this.ctx.fillText(`${tank.number || '?'}`, centerX, centerY - baseFontSize/2);
    
    // Draw tank name
    this.ctx.font = `${Math.max(baseFontSize - 2, 8)}px Arial`;
    this.ctx.fillText(tank.name || 'no name', centerX, centerY + baseFontSize/2);
  }
  ...
  drawTankDetails3D(x, y, tank, width, length, height, scale, isHovered, theme) {
    ...
    this.ctx.textAlign = 'center';
    this.ctx.fillText('Edit', buttonX + buttonWidth/2, buttonY + buttonHeight/2 + buttonFontSize/3);
